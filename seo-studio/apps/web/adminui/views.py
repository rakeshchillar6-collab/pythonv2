...
from .forms import CategoryForm, CorpusForm
...
# --- Vector Search Management Views ---

@login_required
@require_roles('admin', 'editor')
def corpus_list(request: HttpRequest) -> HttpResponse:
    """Displays the list of corpora and handles creation."""
    # TODO: Filter corpora based on user's site access
    corpora = Corpus.objects.all()
    form = CorpusForm() # For the creation modal
    return render(request, 'adminui/vectorsearch/corpus_list.html', {'corpora': corpora, 'form': form})

@login_required
@require_roles('admin', 'editor')
def search_console(request: HttpRequest) -> HttpResponse:
    """Displays the semantic search console."""
    # TODO: Add logic to handle search POST requests via HTMX
    return render(request, 'adminui/vectorsearch/search_console.html')
